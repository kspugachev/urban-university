...
@dp.message_handler(text='Регистрация') # Функция запускается, если нажать кнопку 'Регистрация' или ввести это слово.
async def sing_up(message: types.Message):
	await message.answer(texts.username_reg)
	await RegistrationState.username.set()

@dp.message_handler(state=RegistrationState.username) # Функция запускается, если установлено состояние ('username').
async def set_username(message: types.Message, state: FSMContext):
	if db.is_included(message.text) is False:
		await state.update_data(username=message.text)
		await message.answer(texts.email_reg)
		await RegistrationState.email.set()
	else:
		await message.answer(texts.re_username_reg)
		await RegistrationState.username.set()

@dp.message_handler(state=RegistrationState.email) # Функция запускается, если установлено состояние ('email').
async def set_email(message: types.Message, state: FSMContext):
	await state.update_data(email=message.text)
	await message.answer(texts.age_reg)
	await RegistrationState.age.set()

@dp.message_handler(state=RegistrationState.age) # Функция запускается, если установлено состояние ('age').
async def set_age(message: types.Message, state: FSMContext):
	await state.update_data(age=message.text)
	data_1 = await state.get_data()
	db.add_user(data_1['username'], data_1['email'], data_1['age'])
	await message.answer(f'{texts.reg_info}')
	await state.finish()
...
async def get_buying_list(message: types.Message):
	for i in range(len(res)):
		with open(f'files/file_{i + 1}.jpg', 'rb') as img:
			await message.answer_photo(img, f'{res[i][1]} | {res[i][2]} | {res[i][3]}')
...
@dp.message_handler(state=UserState.age)  # Функция запускается, если установлено состояние ('age').
...
@dp.message_handler(state=UserState.growth)  # Функция запускается, если установлено состояние ('growth').
...
@dp.message_handler(state=UserState.weight)  # Функция запускается, если установлено состояние ('weight').
